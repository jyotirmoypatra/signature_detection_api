...
$signature = json_decode($_POST['data'],true);

   //  $signatureBase64String =  $signature['sign'];
   $signatureBase64String = str_replace(' ', '+', $signature['sign']);
    $uniquenumber = $signature['uniquenumber'];
...
if ($signatureBase64String != '') {
  ...
  // $output = shell_exec("python3 t3.py '$acc'  2>&1");
    $output = shell_exec("source myenv/bin/activate && python3 t3.py '$acc'  2>&1");
